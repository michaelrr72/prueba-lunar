...
function createTournamentApp() {
  ...
  function assignBossToMatch(roundIndex, matchIndex) {
    ...
    const rawPool = globalThis.PruebaLunarData?.BASE_BOSS_POOL ?? [];
    const raw = pickRandom(rawPool);
    match.currentBoss = raw ? {
      id: raw.id,
      name: raw.name,
      icon: raw.enemyIcon ?? '⚔️',
      difficulty: raw.difficulty === 'extreme' ? 'extremo' : (raw.difficulty === 'hard' ? 'dificil' : 'medio')
    } : null;
    saveState();
    renderBracket();
